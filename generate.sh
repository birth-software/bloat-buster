...
mkdir -p build
cd build
cmake --log-level=VERBOSE .. -G Ninja -DCMAKE_CXX_COMPILER=$CLANGXX_PATH -DCMAKE_C_COMPILER=$CLANG_PATH -DCMAKE_LINKER_TYPE=$CMAKE_LINKER_TYPE -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_COLOR_DIAGNOSTICS=ON -DCMAKE_PREFIX_PATH=${CMAKE_PREFIX_PATH}
cd ..
